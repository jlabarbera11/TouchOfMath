...
function setPosition(element, left, top){
	...
	element.css({"position": "absolute", "left": (left - tempParentLeft) + "px", "top": (top - tempParentTop) + "px", "z-index": 0});
}
...
function displayEquation(xml){
	...
	function pseudoMouseDown(target, pageX, pageY){
		...
		if(index == ""){
			...
			$xmlToMove.find("[index]").each(function(i){
	...
	function pseudoMouseMove(pageX, pageY){
		for(i in offsetX){
			$("#" + selector[i]).css({"position": "absolute", "left": (pageX + offsetX[i] - tempParentLeft[i]) + "px", "top": (pageY + offsetY[i] - tempParentTop[i]) + "px"});
		}
	}
	...
	function pseudoMouseUp(target, pageX, pageY){
		function snapBack(){
			for(i in oldLeft){
				$("#" + selector[i]).animate({"left": (oldLeft[i] - tempParentLeft[i]) + "px", "top": (oldTop[i] - tempParentTop[i]) + "px"}, "fast");
			}
		}
	...
	$(".clickable").bind("touchstart", function(event){
		event.preventDefault();
		
		pseudoMouseDown(event.target, event.originalEvent.changedTouches[0].pageX, event.originalEvent.changedTouches[0].pageY);
		
	}).bind("touchmove", function(event){
		event.preventDefault();
		
		pseudoMouseMove(event.originalEvent.changedTouches[0].pageX, event.originalEvent.changedTouches[0].pageY);
		
	}).bind("touchend", function(event){
		pseudoMouseUp(event.target, event.originalEvent.changedTouches[0].pageX, event.originalEvent.changedTouches[0].pageY);
				...
				if(specialDisplayOperator[element.attr("type")]){
					...
					switch(place[element.attr("type")]){
						...
						case 1:		// Operator in between paramters
							output = "<span class='" + elementClass + "'>" + elementToHtml(element.children().eq(0)) + operatorToHtml(element) + elementToHtml(element.children().eq(1)) + "<\/span>";
...
function specialElementToHtml(element){
	...
	switch(element.attr("type")){
		case "/":
			output = "<span class='" + elementClass + "'>" + elementToHtml(element.children().eq(0)) + "<div class='operator parameterDivision2' width='0'><\/div>" + elementToHtml(element.children().eq(1)) + "<\/span>";
			break;
		
		case "sum":
			// Sumation of parameter0 from parameter1 to parameter2
			
			output = "<span class='" + elementClass + "'>" + operatorToHtml(element) + elementToHtml(element.children().eq(1)) + elementToHtml(element.children().eq(2)) + elementToHtml(element.children().eq(0)) + "<\/span>";
			break;
		
		case "parentheses":
			output = "<span class='" + elementClass + "'><span class='operator parameterParentheses1'>(<\/span>" + elementToHtml(element.children().eq(0)) + "<span class='operator parameterParentheses2'>)<\/span><\/span>";
			break;
		
		case "^":
			output = "<span class='" + elementClass + "'>" + elementToHtml(element.children().eq(0)) + elementToHtml(element.children().eq(1)) + "<\/span>";
			break;
		
		default:
			output = "<span id='SDO' class='clickable " + elementClass + "'>UO: \"" + sanitize(element.attr("type")) + "\"<\/span>";
...
function postProcess(){
	$(".operatorPower").each(function(){
		tempTop0 = $(this).children().eq(0).offset().top;
		tempTop1 = $(this).children().eq(1).offset().top;
		
		$(this).children().eq(1).css({"position": "relative", "top": (tempTop0 - tempTop1) + "px"});
	});
	
	$(".operatorSum").each(function(){
		...
		$(this).children(".parameterSum1").eq(0).css({"position": "relative", "left": (-.5 * (tempWidth1 + tempWidthOperator)) + "px"});
		$(this).children(".parameterSum2").eq(0).css({"position": "relative", "left": (-.5 * (tempWidth2 + tempWidthOperator) - tempWidth1) + "px"});
		$(this).children(".parameterSum0").eq(0).css({"position": "relative", "left": (-1 * (tempWidth1 + tempWidth2)) + "px", "margin-right": -(tempWidth1 + tempWidth2) + "px"});
	});
	
	$(".operatorDivision").reverse().each(function(){
		...
		if(tempWidth0 > tempWidth1){
			$(this).children(".parameterDivision1").eq(0).css({"position": "relative", "left": (-.5 * (tempWidth0 + tempWidth1)) + "px", "margin-right": (-tempWidth1) + "px"});
		}else{
		
			// Todo: Investigate the arbitrary offset on the margin
			$(this).children(".parameterDivision0").eq(0).css({"position": "relative", "left": (.5 * (tempWidth0 + tempWidth1) - tempWidth0) + "px", "margin-right": -(tempWidth0 + 4) + "px"});
		}
	...
	$(".parameterParentheses1, .parameterParentheses2").each(function(){
...
function finalize(){
	$(".clickable, .operator, [class*='parameterSum'], [class*='parameterDivision'], [class*='parameterPower']").reverse().each(function(i){
	...
	$(".operatorDivision").each(function(){
		...
		if(tempWidth0 > tempWidth1){
			//$(this).children(".parameterDivision2").eq(0).css({/*"position": "absolute", "left": (tempLeft0 - tempParentLeft) + "px",*/ /*"top": (tempTop1 - tempParentTop) + "px",*/ "width": tempWidth0 + "px"});
			setPosition($(this).children(".parameterDivision2").eq(0), tempLeft0);
			$(this).children(".parameterDivision2").eq(0).css({"width": tempWidth0 + "px"});
		}else{
			//$(this).children(".parameterDivision2").eq(0).css({/*"position": "absolute", "left": (tempLeft1 - tempParentLeft) + "px",*/ /*"top": (tempTop1 - tempParentTop) + "px",*/ "width": tempWidth1 + "px"});
	...
	$(".clickable, .operator").each(function(i){
		...
		if($(this).offset().top < minTop){
			...
			$(".clickable, .operator").each(function(j){		// Shift all elements down
