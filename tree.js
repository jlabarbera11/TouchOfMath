...
function mathTree()
{
	...
	this.buildTree = function(arr) {add(this.root, arr, arr.length-1, 0);
									...
									return printtree(this.root, 0, this.root.children.length-2)};
...
//Returns an array of blocks with inverted operators
{
	...
	var opp = false;
	if(node.children[node.children.length-1-l].inverted)
		opp = true;
	...
	if(assoc[node.value] != 2)
	{
		if(l+arity[node.value] < node.children.length)
		{
			xmlfeed = xmlfeed.concat(printtree(node, l+1, offset-1));
			xmlfeed = xmlfeed.concat(printtree(node.children[node.children.length-1-l], 0, node.children[node.children.length-1-l].children.length-arity[node.children[node.children.length-1-l].value]));
		}
		else
			for(var i=0; i<arity[node.value]; i++)
				xmlfeed = xmlfeed.concat(printtree(node.children[i], 0, node.children[i].children.length-arity[node.children[i].value]));
	}
